...
test("shared footer renders every sector link exactly once in one sector grid", () => {
  ...
  assert.equal((footer.match(/to="\/privacy"/gmu) ?? []).length, 2);
  ...
  assert.match(footer, /if \(compact\)/u);
  assert.match(footer, /Need help booking\?/u);
  assert.match(rootRoute, /<SiteFooter compact=\{path === "\/consult"\} \/>/u);
