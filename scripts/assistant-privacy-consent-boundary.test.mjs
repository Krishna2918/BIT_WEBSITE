...
test("free-text and Turnstile disclosures are proximal while contact consents stay separate", async () => {
  ...
  assert.match(form, /name="email_service_updates_consent" value="yes"/u);
  assert.match(form, /name="whatsapp_service_updates_consent" value="yes"/u);
  assert.doesNotMatch(form, /name="service_update_consent"/u);
  assert.doesNotMatch(form, /name="email_service_updates_consent"[^>]*(?:defaultChecked|checked)/u);
  assert.doesNotMatch(form, /name="whatsapp_service_updates_consent"[^>]*(?:defaultChecked|checked)/u);
