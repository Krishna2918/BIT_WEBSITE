...
import { ConsultIntakeSchema } from "../src/lib/consult-intake-schema.ts";
...
const COMMON = {
  ...
  email_service_updates_consent: true,
  whatsapp_service_updates_consent: false,
...
test("Fleet and Dental canaries use the exact nested bridge contract and remain cross-scope isolated", () => {
  ...
  for (const canary of [fleet, dental]) {
    ...
    assert.equal(canary.body.lead.consent.service_updates.email, true);
    assert.equal(canary.body.lead.consent.service_updates.whatsapp, false);
...
test("service routing is deterministic and mixed work creates Consultant collaboration", () => {
  ...
  assert.equal(software.body.lead.consent.service_updates.whatsapp, false);

  const whatsapp = buildFormAiCrmPayload(
    {
      ...COMMON,
      submission_id: "78787878-7878-4878-8878-787878787878",
      intent: "general",
      source: "consult-page",
      interest: "General consultation",
      services: ["AI workflows"],
      preferred_reply: "WhatsApp",
      email_service_updates_consent: false,
      whatsapp_service_updates_consent: true,
    },
    received,
    STAFFED_WINDOW,
  );
  assert.equal(whatsapp.body.lead.consent.service_updates.email, false);
  assert.equal(whatsapp.body.lead.consent.service_updates.whatsapp, true);
  ...
  assert.throws(
    () =>
      buildFormAiCrmPayload(
        {
          ...COMMON,
          submission_id: "79797979-7979-4979-8979-797979797979",
          intent: "general",
          source: "consult-page",
          interest: "General consultation",
          services: ["AI workflows"],
    ...
    /email reply requires email service-update consent/,
  );
...
test("service-update consent is explicit and isolated per preferred channel", () => {
  const base = {
    ...COMMON,
  ...
  const phoneOnly = buildFormAiCrmPayload(
    {
      ...base,
      preferred_reply: "Phone",
      email_service_updates_consent: false,
  ...
  assert.deepEqual(phoneOnly.body.lead.consent.service_updates, {
    email: false,
    whatsapp: false,
  });
});
