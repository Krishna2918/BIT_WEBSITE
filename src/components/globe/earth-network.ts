...
export function buildStipple(params: NetworkParams): NetworkBuffers {
  ...
  const mask = rasterizeLandMask(TEX_W, TEX_H);
  const fibN = Math.min(Math.max(nodeBudget * 2.4, 36000), 160000);

  const xs: number[] = [];
  const ks: number[] = [];

  const emit = (x0: number, y0: number, z0: number, kind: number, seed: number) => {
    let x = x0 + (hash2(seed, 1.3) - 0.5) * 0.008;
    let y = y0 + (hash2(seed, 4.7) - 0.5) * 0.008;
    let z = z0 + (hash2(seed, 8.2) - 0.5) * 0.008;
    const len = Math.hypot(x, y, z) || 1;
    x /= len;
    y /= len;
    z /= len;
    const lift = 0.003 + kind * 0.007;
  ...
  for (let i = 0; i < fibN; i++) {
    ...
    if (land > 0.38) {
      emit(x, y, z, Math.min(1, 0.7 + land * 0.35), i);
